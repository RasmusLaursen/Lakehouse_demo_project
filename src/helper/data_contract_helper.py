...
def get_data_contract_path(catalog: str, object_name: str) -> Path:
    ...
    possible_paths = [
        Path(f"src/data_contracts/{catalog}/{object_name}.yml"),  # From project root
        Path(f"../data_contracts/{catalog}/{object_name}.yml"),   # From src/helper or src/base
        Path(f"../../data_contracts/{catalog}/{object_name}.yml"), # From nested directories
        Path(__file__).parent.parent / "data_contracts" / catalog / f"{object_name}.yml"  # Absolute from this file
    ]
