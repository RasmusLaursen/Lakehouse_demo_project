"""Synthetic data generation using Faker and ODCS data contracts."""
...
from src.framework.helper.contracts import load_data_contract
...
class DynamicFakeDataGenerator:
    ...
    def __init__(self, data_contract_file: str, locale: str = 'en_US'):
        ...
        self.data_contract = load_data_contract(Path(data_contract_file))
        self.schemas = self._get_all_schemas(self.data_contract)
        logger.info(f"Loaded data contract with {len(self.schemas)} schemas")
        self.generated_ids = {}  # Track generated IDs for foreign keys
    
    def _get_all_schemas(self, data_contract: OpenDataContractStandard) -> List[SchemaObject]:
        """Extract all schemas from data contract."""
        if hasattr(data_contract, 'schema_') and data_contract.schema_:
            return data_contract.schema_
        return []
