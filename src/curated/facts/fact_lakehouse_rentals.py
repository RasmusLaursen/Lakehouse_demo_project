...
def fact_lakehouse_rentals(
    ...
    lakehouse_rentals_df = dimension_keys_lookup(curated_catalog, curated_dimension_schema, fact_df = lakehouse_rentals_df)
...
def dimension_keys_lookup(curated_catalog, curated_dimension_schema, fact_df):
    ...
    for key in dimenension_keys:
        if key in fact_df.columns:
            ...
            fact_df = fact_df.alias("lhr").join(
                dimension_df.select(col(dimension_key).alias(key), col(f"{dimension_name}_id").alias(f'{dimension_role_name}_id')).alias("dim"),
    ...
    cols = [c for c in fact_df.columns if c.endswith('_id')] + [c for c in fact_df.columns if not c.endswith('_id')]
    fact_df = fact_df.select(*cols)
    return fact_df