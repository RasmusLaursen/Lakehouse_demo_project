    ...
    return next(
        filter(
            ...
            and (
                ...
                or
                job.settings.tags["environment"] == os.getenv("DATABRICKS_ENVIRONMENT", "dev")
            )
...
def step_impl(context):
    try:
        context.run = context.api.run_now_and_wait(job_id=context.job_id, timeout=datetime.timedelta(seconds=TIMEOUT))
...
def step_impl(context):
    ...
    if context.run.state.result_state != "SUCCESS":
        raise Exception(f"Job did not complete successfully. Result state: {context.run.state.result_state}")
